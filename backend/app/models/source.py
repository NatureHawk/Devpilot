...
class CodeChunk(Base, UUIDPrimaryKey, Timestamps):
    ...
    __table_args__ = (
        ...
        # Lexical retrieval: `search_vector @@ query`.
    ...
    # Normalised full-text terms (see app.services.lexical): path and symbol
    # names, and content. Written by indexing; empty for rows indexed before
    # lexical search existed.
    lexical_names: Mapped[str] = mapped_column(Text, nullable=False, default="", server_default="")
    lexical_body: Mapped[str] = mapped_column(Text, nullable=False, default="", server_default="")
    # Generated by PostgreSQL from the two fields above, names weighted A and
    # body D. Never written by the application.
